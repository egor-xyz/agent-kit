...
Then invoke as `/handoff` and `/handoff-resume`.
...
```
   work...
     │
     ▼
  /handoff         ─▶  writes .agents/handoff.md (target, state, files, changes, dead ends, next step)
     │
     ▼
   /clear                    ─▶  fresh context
     │
     ▼
   /handoff-resume ─▶  reads .agents/handoff.md, verifies git state, echoes next step, WAITS
     │
     ▼
...
| --- | --- | --- | --- | --- |
| [`agent-kit`](./plugins/agent-kit) | Portable workflows — starting with session handoff. | `/handoff`, `/handoff-resume` | `auto-handoff` | all |
...
- **Handoff-native.** Sessions are disposable. Every workflow that produces stateful work documents how it integrates with `/handoff`.
